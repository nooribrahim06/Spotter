...
export async function createProgressEntry(req, res) {
  const progressEntry = await progressService.createProgressEntry(
    req.validatedBody,
    req.user.id
  );
...
export async function getProgressHistory(req, res) {
  const progressHistory = await progressService.getProgressHistory(
    req.user.id,
    req.validatedQuery,
    req.user.timezone
  );
...
export async function getLatestProgressEntry(req, res) {
  const latestProgressEntry = await progressService.getLatestProgressEntry(
    req.user.id
  );
...
export async function getProgressEntryById(req, res) {
  const progressEntry = await progressService.getProgressEntryById(
    req.user.id,
    req.validatedParams.entryId
  );
  return privateResponse(res, 200, progressEntry);
}

export async function getGoalProgress(req, res) {
  const goalProgress = await progressService.getGoalProgress(req.user.id);
  return privateResponse(res, 200, goalProgress);
}
